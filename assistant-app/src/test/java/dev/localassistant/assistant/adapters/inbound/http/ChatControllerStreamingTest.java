...
class ChatControllerStreamingTest {
    ...
    void streamsFinalEventWithStructuredAnswer() throws Exception {
        when(answerQuestionUseCase.answer(any(UserQuestion.class), any(AssistantResponseSink.class)))
                .thenAnswer(
                        invocation -> {
                            AssistantResponseSink sink = invocation.getArgument(1);
                            ...
                            sink.recordSourceOutcome(
                                    dev.localassistant.assistant.orchestration.SourceType.COUNTRIES_FACTS,
                                    dev.localassistant.assistant.question.SourceContributionStatus.USED);
                            sink.complete(turn);
                            return turn;
                        });
    ...
    void rejectedExecutorEmitsErrorEventOverOpenStream() throws Exception {
        ...
        List<SseEvent> events = parseSseEvents(completed.getResponse().getContentAsString());
        assertThat(events).extracting(SseEvent::name).containsExactly("error");
        verify(answerQuestionUseCase, never()).answer(any(UserQuestion.class), any(AssistantResponseSink.class));
    }
