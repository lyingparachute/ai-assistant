...
final class SourceRoutingPolicy {
    ...
    RoutedQuestion route(final UserQuestion question) {
        final var normalized = question.text().toLowerCase(Locale.ROOT);
    ...
    private static boolean containsCountryThenWeather(final String normalized) {
        return normalized.contains(KEYWORD_TEMPERATURE)
            && normalized.contains(KEYWORD_CAPITAL)
            && normalized.contains(KEYWORD_GERMANY);
    }

    private static boolean containsCountryCapital(final String normalized) {
        return normalized.contains(KEYWORD_CAPITAL) && normalized.contains(KEYWORD_GERMANY);
    }

    private static boolean containsWeatherLocation(final String normalized) {
        return normalized.contains(KEYWORD_TEMPERATURE) && normalized.contains(KEYWORD_MUNICH);
    }

    private static boolean startsWithPlaceSynthesisPrefix(final String normalized) {
        return normalized.startsWith(PLACE_SYNTHESIS_PREFIX);
    }

    private static boolean containsCdqProduct(final String normalized) {
        return normalized.contains(KEYWORD_CDQ) || normalized.contains(KEYWORD_FRAUD_GUARD);
    }

    private static String extractPlaceName(final String questionText) {
        final var normalized = questionText.toLowerCase(Locale.ROOT);
    ...
    private static String stripTrailingSentencePunctuation(final String value) {
    ...
    private static boolean isSentencePunctuation(final char character) {
